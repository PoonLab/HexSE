...
def get_evol_rates(seq, mu, bias, pi, orfs):
    """
    ...
    omega = get_omega(orfs)
    syn_nonsyn = get_syn_subs(seq, orfs, codon_dict)
    ...
    for position in range(len(seq)):
        ...
        for to_nt in nts:
            if to_nt == my_nt:
                ...
                mutation[to_nt] = mu * bias[my_nt][to_nt] * pi[my_nt]

        evol_rates.append(mutation)
        ...
        for i in range(len(orfs)):
            ...
            if position in range(orf[0],orf[1]+1):
                ...
                for to_nt in nts:
                    if evol_rates[position][to_nt] != None:
